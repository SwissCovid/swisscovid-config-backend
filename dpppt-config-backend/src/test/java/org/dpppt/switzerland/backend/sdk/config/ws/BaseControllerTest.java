...
public abstract class BaseControllerTest {
    ...
    public void testGetConfig() throws Exception {
        mockMvc.perform(get("/v1/config")).andExpect(status().is4xxClientError());
        ...
        Jwts.parserBuilder()
                ...
                .build()
                .parse(result.getHeader("Signature"));
    }
    ...
    public void testForUpdateNote() throws Exception {
        ...
        result =
                mockMvc.perform(
                                get("/v1/config")
                                        .param("osversion", "ios12")
                                        .param("appversion", "android-1.0.1")
                                        .param("buildnr", "ios-2020.0145asdfa34"))
                        .andExpect(status().is2xxSuccessful())
                        .andReturn()
                        .getResponse();
        ...
        result =
                mockMvc.perform(
                                get("/v1/config")
                                        .param("osversion", "ios12")
                                        .param("appversion", "android-1.0.4")
                                        .param("buildnr", "ios-2020.0145asdfa34"))
                        .andExpect(status().is2xxSuccessful())
                        .andReturn()
                        .getResponse();
    ...
    public void testForTestflight() throws Exception {
        final List<String> testflightVersions =
                List.of(
                        "ios-200619.2333.175",
                        "ios-200612.2347.141",
                        "ios-200528.2230.100",
        ...
        final MockHttpServletResponse result =
                mockMvc.perform(
                                get("/v1/config")
                                        .param("osversion", "ios12")
                                        .param("appversion", "1.0.7")
                                        .param("buildnr", "ios-200521.2320.80"))
                        .andExpect(status().is2xxSuccessful())
                        .andReturn()
                        .getResponse();
        assertTestNoUpdate(result);
    ...
    public void testWhatToDoPositiveTestTextLanguage() throws Exception {
        ...
        assertEquals(
                "Noch keinen Covidcode?",
                whatToDoPositiveTestTexts.getDe().getInfoBox().getTitle());
        assertEquals(
                "Pas encore de code COVID ?",
                whatToDoPositiveTestTexts.getFr().getInfoBox().getTitle());
        assertEquals(
                "Non hai ancora un codice Covid?",
                whatToDoPositiveTestTexts.getIt().getInfoBox().getTitle());
        assertEquals(
                "Don’t have a Covidcode yet?",
                whatToDoPositiveTestTexts.getEn().getInfoBox().getTitle());
        assertEquals(
                "Ainda não recebeu um código COVID?",
                whatToDoPositiveTestTexts.getPt().getInfoBox().getTitle());
        assertEquals(
                "¿Aún no ha recibido el código Covid?",
                whatToDoPositiveTestTexts.getEs().getInfoBox().getTitle());
        assertEquals(
                "Nuk keni marrë ende asnjë kod Covid?",
                whatToDoPositiveTestTexts.getSq().getInfoBox().getTitle());
        assertEquals(
                "Još niste dobili Covid šifru?",
                whatToDoPositiveTestTexts.getBs().getInfoBox().getTitle());
        assertEquals(
                "Još niste dobili Covid šifru?",
                whatToDoPositiveTestTexts.getHr().getInfoBox().getTitle());
        assertEquals(
                "Još niste dobili Covid šifru?",
                whatToDoPositiveTestTexts.getSr().getInfoBox().getTitle());
        assertEquals(
                "Anc nagin code covid?", whatToDoPositiveTestTexts.getRm().getInfoBox().getTitle());
        assertEquals(
                "Covid kodunu henüz almadınız mı?",
                whatToDoPositiveTestTexts.getTr().getInfoBox().getTitle());
        assertEquals(
                "ናይ ኮቪድ ኮድ ገና የብልኩምን፧", whatToDoPositiveTestTexts.getTi().getInfoBox().getTitle());
    }
    ...
    public void testTestLocations() throws Exception {
        ...
        for (int i = 0; i < allCantonsAndLiechtenstein.size(); i++) {
            assertEquals(
                    resp.getTestLocations().getRm().get(i).getRegion(),
                    allCantonsAndLiechtenstein.get(i));
        }
    }
